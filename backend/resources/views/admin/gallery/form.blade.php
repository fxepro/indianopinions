...
@section('page_title', $gallery ? 'Edit Image' : 'Add Image')

@section('content')
@php($initialImageUrl = old('image_url', $gallery?->image_url ?? ''))

<x-admin.page-header :title="$gallery ? 'Edit Image' : 'Add Image'">
    <x-slot:actions>
        <a href="{{ admin_route('admin.gallery.index') }}" class="btn btn-outline">Back</a>
    </x-slot:actions>
</x-admin.page-header>

<form method="POST" action="{{ $gallery ? admin_route('admin.gallery.update', $gallery) : admin_route('admin.gallery.store') }}" class="max-w-2xl">
    @csrf
    @if($gallery) @method('PUT') @endif

    <div class="space-y-5">

        {{-- Image URL + preview --}}
        <div class="bg-white rounded-xl border border-zinc-200 p-6 space-y-4"
             x-data="imageUrlPreview(@js($initialImageUrl))">
        ...
        <div class="flex gap-3">
            <button type="submit" class="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg text-sm transition">
                {{ $gallery ? 'Update Image' : 'Add Image' }}
            </button>
...
@push('scripts')
<script>
function imageUrlPreview(initialUrl) {
    return {
        url: initialUrl || '',
    };
}
